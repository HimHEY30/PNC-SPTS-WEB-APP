...
function handleSave() {
  ...
  if (!formData.value.phone.trim()) validationErrors.phone = 'Phone number is required'
...
<template>
  ...
  >
    ...
    <div 
      @click.stop
      class="bg-white rounded-[8px] w-full max-w-lg shadow-xl border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden"
    >
      <!-- Header -->
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input 
              ...
              type="text" 
              class="w-full rounded-[3px] border px-3 py-1.5 text-xs text-slate-700 placeholder-slate-400 focus:outline-none bg-white"
              :class="errors.phone ? 'border-red-300 focus:border-red-400' : 'border-slate-200 focus:border-slate-400'"
