...
const followUps = ref([
  {
    ...
    comment: '<p>Student has shown significant improvement in recent <strong>JavaScript</strong> assignments. Participation has also increased.</p>',
  ...
  },
  {
    ...
    comment: '<p>Phanith has missed 3 sessions this week without prior notification. Need to contact parents for clarification.</p>',
    attachments: []
  },
  {
    id: 3,
    ...
    comment: '<p>Awarded "Student of the Month" for outstanding contribution to the <strong>Environmental Awareness</strong> campaign.</p>',
  ...
  },
  {
    ...
    comment: '<p>Disruptive conduct reported during the Chemistry lab session. Safety protocols were violated. Immediate intervention required.</p>',
...
<template>
  <div class="space-y-6 text-left max-w-[1000px] mx-auto pb-10">
    ...
    <div class="flex items-center justify-between">
      ...
      <button class="flex items-center gap-2 px-4 py-2 bg-[#3b4b6b] hover:bg-[#2e3b54] text-white font-bold text-xs rounded-[5px] transition-colors shadow-sm">
    ...
    <div class="bg-white rounded-[5px] border border-slate-100 p-4 shadow-md flex flex-wrap items-center justify-between gap-4">
      <div class="flex flex-wrap items-center gap-3">
        <BaseDropdown v-model="filterType" :options="typeOptions" placeholder="All Types" />
        <BaseDropdown v-model="filterPriority" :options="priorityOptions" placeholder="All Priorities" />
        ...
        <button 
          ...
          @click="filterType = ''; filterPriority = ''; filterStatus = ''"
    ...
    <div class="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
      ...
      >
        <!-- Dot indicator -->
        <div class="flex items-center justify-center w-10 h-10 rounded-full border border-white bg-slate-100 text-slate-400 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-colors group-hover:bg-indigo-600 group-hover:text-white">
          <IconCalendar class="w-5 h-5" />
        </div>

        <!-- Card content -->
        <div class="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-5 rounded-[5px] border border-slate-100 shadow-md group-hover:shadow-lg transition-all">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              <div class="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-500">
            ...
            <div class="flex items-center gap-1">
              <button class="p-1.5 rounded-[5px] text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 transition-all">
                <IconEdit class="w-4 h-4" />
              </button>
              <button class="p-1.5 rounded-[5px] text-slate-300 hover:text-rose-600 hover:bg-rose-50 transition-all">
          ...
          <div class="space-y-3">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span :class="['px-2 py-0.5 rounded-[3px] text-[9px] font-bold uppercase tracking-wider border', getPriorityClass(item.priority)]">
                {{ item.priority }} Priority
              </span>
              <span :class="['px-2 py-0.5 rounded-[3px] text-[9px] font-bold uppercase tracking-wider border', getStatusClass(item.status)]">
                {{ item.status }}
              </span>
              <span class="px-2 py-0.5 rounded-[3px] text-[9px] font-bold uppercase tracking-wider border bg-slate-50 text-slate-500 border-slate-100">
                {{ item.type }}
              </span>
            </div>
            ...
            <div class="text-xs text-slate-600 leading-relaxed font-medium prose prose-sm max-w-none" v-html="item.comment"></div>
    ...
    <div class="pt-8 flex justify-center">
      <button class="px-6 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-xs rounded-[5px] transition-all shadow-sm">
        Load More Records
      </button>
    </div>
  </div>
